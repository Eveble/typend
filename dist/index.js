...
class InstanceOf extends WrapperPattern {
    ...
    isValid(type) {
        ...
        return (type == null ||
            helpers.isClass(type) ||
            helpers.isConstructor(type) ||
            helpers.isSinonClockDate(type) ||
...
function getResolvablePath(path, properties) {
    ...
    for (const pathPart of pathParts.reverse()) {
        ...
        const isNested = !lodash.has(properties, nearestPath);
...
function isResolvablePath(key, properties) {
    return lodash.has(properties, key);
}
function getMatchingParentProto(childProto, matcher) {
    const parentProtos = polytype.getPrototypeListOf(childProto);
...
class ClassValidator extends PatternValidator {
    ...
    validate(value, pattern, validator) {
        if (!helpers.isClassInstance(value)) {
            throw new InvalidTypeError('Expected %s to be an instance of %s', this.describe(value), helpers.getTypeName(pattern.type));
        }
        const properties = pattern.properties;
        const differences = deepDiff.diff(properties, value);
        ...
        for (const difference of differences) {
            ...
            const propertiesFromPath = lodash.get(properties, key);
            try {
        ...
        if (!(value instanceof pattern.type)) {
            throw new UnmatchedTypeError(`Expected %s to be a instance of %s`, this.describe(value), helpers.getTypeName(pattern.type));
        }
        return true;
    }
}
...
class CollectionIncludingValidator extends PatternValidator {
    ...
    validate(value, collIncOrExpect, validator) {
        ...
        for (const difference of differences) {
            ...
            const valueFromPath = lodash.get(value, key);
            const expectationFromPath = lodash.get(collIncOrExpect, key);
            try {
...
class CollectionWithinValidator extends PatternValidator {
    ...
    validate(value, collectionWithin, validator) {
        ...
        for (const difference of differences) {
            ...
            const valueFromPath = lodash.get(value, key);
            const expectationFromPath = lodash.get(collectionWithin, key);
            try {
                validator.validate(valueFromPath, expectationFromPath);
            }
            catch (err) {
...
class InstanceOfValidator extends PatternValidator {
    canValidate(expectation) {
        ...
        if (lodash.isArray(expectation)) {
            return false;
        }
        return (expectation == null ||
            helpers.isClass(expectation) ||
            helpers.isConstructor(expectation) ||
            helpers.isSinonClockDate(expectation) ||
            [Symbol].includes(expectation));
    }
    validate(value, instanceOfOrExpect) {
        ...
        else if (helpers.isSinonClockDate(expectation)) {
            isValid = true;
        }
...
class InterfaceValidator extends PatternValidator {
    ...
    validate(value, pattern, validator) {
        ...
        const differences = deepDiff.diff(pattern, value);
        if (differences === undefined || lodash.isEmpty(differences)) {
            return true;
        }
        for (const difference of differences) {
            ...
            const valueFromPath = lodash.get(value, key);
            const expectationFromPath = lodash.get(pattern, key);
            try {
                validator.validate(valueFromPath, expectationFromPath);
            }
            catch (err) {
...
class TupleValidator extends PatternValidator {
    canValidate(expectation) {
        return (expectation instanceof Tuple ||
            (lodash.isArray(expectation) && expectation.length > 0));
    }
...
class ErrorDescriber {
    describe(arg) {
        let errorMessage;
        let type;
        if (helpers.isErrorInstance(arg)) {
            errorMessage = `'${arg.message}'`;
            type = helpers.getTypeName(arg.constructor);
        }
        else {
            type = helpers.getTypeName(arg);
        }
...
class NativeTypeDescriber {
    ...
    describeType(arg) {
        ...
        if (arg === undefined) {
            ...
            type = helpers.getNativeType(arg).name;
        }
        return type;
    }
    generateMessage(value, type) {
...
class CompactDescriber {
    ...
    describe(arg) {
        const message = util.inspect(arg, this.options);
        return new Description({
            message,
        });
    }
...
class ClassDescriber {
    ...
    describeType(arg) {
        if (helpers.isClass(arg)) {
            return helpers.getTypeName(arg);
        }
        return helpers.getTypeName(arg.constructor);
    }
...
class FallbackDescriber {
    ...
    describeType(arg) {
        return lodash.capitalize(typeof arg);
    }
    generateMessage(value, type) {
        let msg;
        if (type && value) {
...
class ClassConverter {
    ...
    isConvertible(reflectedTypeOrClass) {
        return (this.isReflectedReference(reflectedTypeOrClass) ||
            ...
            helpers.isClass(reflectedTypeOrClass));
    }
    convert(reflectedType, converter) {
    ...
    reflectClassType(type) {
        const reflectedClass = tsruntime.getClassType(type);
    ...
    isReflectedReference(reflectedType) {
        ...
        return referenceType.type !== undefined && helpers.isClass(referenceType.type);
    }
...
class Describer {
    ...
    createDescription(value) {
        if (helpers.isMultidimensionalArray(value)) {
...
exports.CollectionIncluding = CollectionIncluding;
exports.CollectionIncludingValidator = CollectionIncludingValidator;
exports.CollectionValidator = CollectionValidator;
exports.CollectionWithin = CollectionWithin;
exports.CollectionWithinValidator = CollectionWithinValidator;
exports.CompactDescriber = CompactDescriber;
exports.DebugDescriber = DebugDescriber;
...
exports.describer = describer;
exports.eq = eq;
exports.equivalentClassChai = equivalentClassChai;
